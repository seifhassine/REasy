...
class CompactMotClipV27Writer:
    ...
    def build(self, clip: CompactMotClip, *, origin_offset: int) -> bytes:
        ...
        for prop, key in [*key_entries, *last_entries]:
            if prop.property_type == ClipPropertyType.ENUM:
                ascii_key_offsets[id(key)] = self._pool_size(ascii_values, "ascii")
                ascii_values.append(str(key.value))
            elif prop.property_type in (ClipPropertyType.STR16, ClipPropertyType.GUID):
                unicode_key_offsets[id(key)] = self._pool_size(unicode_values, "utf-16le") // 2
                unicode_values.append(str(key.value))
        ...
        for index, prop in enumerate(props):
            ...
            struct.pack_into("<iffII", record, 0, prop.array_index, prop.start_frame, prop.end_frame, 2, len(prop.speed_points))
            ...
            struct.pack_into("<QQQ", record, 0x48, member_start, member_count, last_index[id(prop.last_key)] if prop.last_key else 0)
        ...
        for index, extra in enumerate(clip.extra_ranges):
            ...
            values_start += sum(len(item.intervals) for item in clip.extra_ranges[:index]) * 8
            struct.pack_into(
                "<IhhQ",
                out,
                record,
        ...
        for extra in clip.extra_ranges:
            for interval in extra.intervals:
                begin_bits = 0xFFFFFFFF if interval.begin_frame is None else struct.unpack("<I", struct.pack("<f", interval.begin_frame))[0]
        ...
        for index, name in enumerate(
            ("nodes", "properties", "keys", "speed", "hermite", "bezier", "legacy_unused", "last", "ascii", "unicode", "oword", "extra")
        ):
            struct.pack_into("<Q", out, 0x28 + index * 8, stored(table_offsets[name]))
        return bytes(out)
    ...
    def _curve_index(item, hermite_index, bezier_index) -> int:
        if item.interpolation == ClipInterpolation.HERMITE:
    ...
    @staticmethod
    def _encode_value(property_type, key, ascii_offsets, unicode_offsets, oword_offsets) -> int:
        value = key.value
        if property_type == ClipPropertyType.BOOL:
            return int(value)
        if property_type == ClipPropertyType.S32:
            return struct.unpack("<Q", struct.pack("<q", int(value)))[0]
        if property_type == ClipPropertyType.U32:
            if not 0 <= value <= 0xFFFFFFFF:
                raise MotionWriteError("U32 compact key is out of range")
            return value
        if property_type == ClipPropertyType.F32:
            return struct.unpack("<Q", struct.pack("<d", float(value)))[0]
        if property_type == ClipPropertyType.ENUM:
            return ascii_offsets[id(key)]
        if property_type in (ClipPropertyType.STR16, ClipPropertyType.GUID):
    ...
    @staticmethod
    def _pool_size(values: list[str], encoding: str) -> int:
        terminator = 2 if encoding == "utf-16le" else 1
        return sum(len(value.encode(encoding)) + terminator for value in values)
