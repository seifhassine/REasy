"""Clone request userdata and its native shape group with an independent hit reference."""
import argparse
import contextlib
import io
...
def resolve_requests(rcol):
    ...
    for index, request in enumerate(rcol.request_sets):
        ...
        result[request.info.id] = {
            'name': request.info.name, 'field0': request.info.field0, 'group': str(group.info.guid),
...
def configure(parser):
    ...
    parser.add_argument('--field0', type=int, help='new hit reference (default: max field0 + 1)')
    template = parser.add_mutually_exclusive_group(required=True)
    template.add_argument('--template', help='unique template request name')
    template.add_argument('--template-id', type=int, help='template request ID')
...
def run(args):
    ...
    templates = [index for index, request in enumerate(rcol.request_sets)
                 if (request.info.id == args.template_id if args.template_id is not None else request.info.name == args.template)]
    ...
    shape_count = len(shape_names)      # the composite box count is weapon-specific (LS 3, ChargeAxe 2)
    used_field0 = {request.info.field0 for request in rcol.request_sets}
    field0 = max(used_field0, default=-1)+1 if args.field0 is None else args.field0
    if field0 in used_field0 or not 0 <= field0 <= 0xFFFFFFFF:
        raise ValueError('New field0 must be an unused unsigned 32-bit hit reference')
    ...
    assert len(after[new_id]['shapes']) == shape_count
    ...
    assert reopened.rcol.request_sets[new_index].info.field0 == field0
