"""Clone the LongSword atk_112 attack node into a brand-new node (atk_620).
...
usage: python clone_attack_node.py SOURCE OUTPUT_DIR [--name atk_620] [--motion 620]
"""
import argparse
import copy
import hashlib
...
from file_handlers.motfsm.motfsm_file import MotfsmFile, BHVTNode, ChildNode, Action  # noqa: E402
from file_handlers.motfsm.rebuild import BhvtPointers                                # noqa: E402
from file_handlers.motfsm.rsz_adapter import BLOCK_NAMES                             # noqa: E402
from test_motfsm_rebuild import variable_snapshot                                    # noqa: E402
from utils.hash_util import murmur3_hash                                             # noqa: E402
...
parser.add_argument('--template', default='0E306ED1', help='template attack node identity (hex)')
parser.add_argument('--parent', default='atk')
parser.add_argument('--name', default='atk_620')
parser.add_argument('--motion', type=int, default=620)
parser.add_argument('--keep-hit-index', type=int, default=29)
...
TEMPLATE_ID = int(args.template, 16)
template_index = next(i for i, n in enumerate(nodes) if n.id_hash == TEMPLATE_ID and n.ex_id == 0)
template = nodes[template_index]
parent_index = next(i for i, n in enumerate(nodes) if n.name == args.parent)
parent = nodes[parent_index]
assert not any(n.name == args.name for n in nodes), f'node name already used: {args.name}'
assert template.name == 'atk_112' and len(template.actions) == 12, (template.name, len(template.actions))
...
for (position, reference, instance), new_id in zip(kept, new_action_ids):
    ...
    assert block_name == 'actions', block_name
    extra = {'v1_ID': new_id}
...
new_string = (args.name + '\0').encode('utf-16le')
assert len(new_string) % 16 == 0, 'name must keep the string pool 16-byte aligned'
...
def serialize_node(n):
    ...
    for t in n.transitions:
        data += array(t.mStartTransitionEvent.values, 'i') if doc.layout.transition_event_lists is not False \
...
def serialize_nodes():
    return pack('I', len(nodes)) + b''.join(map(serialize_node, nodes)) + \
        array(doc.bhvt.action_ex_ids) + array(doc.bhvt.static_action_ex_ids)
...
assert serialize_nodes() == source[nodes_start:doc.bhvt.node_data_end], 'node writer does not reproduce the source'
nodes.append(new_node)
parent.children.append(ChildNode(new_node_id, 0, -1))
doc.bhvt.action_ex_ids = list(doc.bhvt.action_ex_ids) + [0] * len(new_action_refs)
...
for start, end, payload in splices:
    assert cursor <= start <= end, (cursor, start, end)
...
assert variable_snapshot(verified) == variable_snapshot(original), 'UVAR relocation changed'
...
assert motion['v4_MotionID'].value == args.motion and motion['v3_BankID'].value == 100, motion
assert [s.TransitionConditions & 0xFFFFFFFF for s in clone.states] == new_state_conditions
...
manifest = dict(
    source=str(TARGET), source_sha256=source_hash, source_size=len(source),
    candidate=str(candidate), candidate_sha256=hashlib.sha256(output).hexdigest(), candidate_size=len(output),
    template=dict(index=template_index, identity=f'0x{TEMPLATE_ID:08X}', name=template.name),
    node=dict(index=new_node_index, identity=f'0x{new_node_id:08X}', name=args.name,
              name_hash=f'0x{clone.name_hash:08X}', fullname_hash=f'0x{clone.fullname_hash:08X}',
              name_index=string_count, parent=parent.name, parent_identity=f'0x{parent.id_hash:08X}',
              selector_id=clone.selector_id, selector_mode=args.selector),
    actions=dict(kept=[dict(position=p, class_name=i.class_name, new_id=f'0x{v:08X}')
                       for (p, _, i), v in zip(kept, new_action_ids)],
                 dropped=[dict(position=p, class_name=c, reason=r) for p, c, r in dropped],
                 motion_instance=motion_instance),
    conditions=dict(new_ids=[f'0x{v:08X}' for v in new_condition_ids],
                    raws=[f'0x{v:08X}' for v in new_state_conditions]),
    transition_events=dict(template_objects=[list(s.mStates.values) for s in template.states],
                           new_objects=new_state_events),
    transition_maps=dict(template_ids=[s.TransitionMaps for s in template.states],
                         new_ids=new_state_transitions, new_map_entries=added_maps),
    counts=dict(nodes=[len(original.bhvt.nodes), len(verified.bhvt.nodes)],
                action_objects=[len(old_objects), len(new_block.object_table)],
                conditions=[len(original.rsz_blocks.get_block('conditions').object_table),
                            len(verified.rsz_blocks.get_block('conditions').object_table)],
                static_conditions=[len(original.rsz_blocks.get_block('static_conditions').object_table),
                                   len(verified.rsz_blocks.get_block('static_conditions').object_table)],
                transition_events=[len(old_events.object_table),
                                   len(verified.rsz_blocks.get_block('transition_events').object_table)],
                transition_maps=[original.transition_map_count, verified.transition_map_count],
                transition_data=[original.transition_data_count, verified.transition_data_count]),
    dropped_by_request=['children', 'transitions'],
)
