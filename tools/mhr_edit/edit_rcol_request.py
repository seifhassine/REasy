...
usage: python edit_rcol_request.py RCOL OUTDIR --id 72 --set _HitStartDelay=38 --set _HitEndDelay=9
"""
import argparse
import contextlib
import hashlib
...
parser.add_argument('--id', dest='request_id', type=int, required=True)
...
matches = [index for index, request in enumerate(rcol.request_sets) if request.info.id == args.request_id]
assert len(matches) == 1, f'request id {args.request_id} resolves to {len(matches)} entries'
index = matches[0]
...
(folder / 'manifest.json').write_text(json.dumps(dict(
    ...
    request_set_id=args.request_id, fields={name: value for name, value in args.assignments},
    size_delta=len(output) - len(source_bytes)), indent=2), encoding='utf-8')
